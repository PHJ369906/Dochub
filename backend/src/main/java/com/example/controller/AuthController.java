...
public class AuthController {
    ...
    public ResponseEntity<String> login(@Valid @RequestBody LoginRequest request) {
        try {
            ...
            String json = String.format(
                "{\"code\":200,\"message\":\"登录成功\",\"data\":{\"token\":\"%s\",\"user\":{\"id\":%d,\"username\":\"%s\",\"email\":\"%s\",\"role\":\"%s\",\"enabled\":%s}}}",
                result.get("token"),
                user.getId(),
                user.getUsername(),
    ...
    public Result<IPage<UserResponse>> getUserList(
        ...
        try {
            ...
            IPage<UserResponse> users = userService.getUserList(page, username, role);
    ...
    public Result<String> toggleUserStatus(@PathVariable Long id) {
        try {
            String result = userService.toggleUserStatus(id);
